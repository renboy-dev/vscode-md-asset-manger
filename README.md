...
- 使用Obsidian语法，资源目录可以自由修改，在配置`mdAssetManager.assetsRoot`后也可以正常找到文件，也不会引起笔记中链接丢失

有个这个插件你可以这么玩笔记：
- 电脑端本地笔记方案：
  - VSCode+Markdown Asset Manager（本地附件集中管理）+Git（笔记同步到远端）
  - Obsidian+Git（笔记同步到远端）
- 手机端本地笔记方案：Obsidian+Git（笔记同步到远端）

为什么要开发这个插件那？主要我是一名IT工作者，有大量的md笔记和其他类型文件`pdf、ppt、excel`的参考资料，VSCode文字排版美观和丰富插件生态，让笔记不局限于md文件，其能够支持很多类型的文件。使用了Git的远端存储让笔记不仅仅是笔记，是有了版本控制能力的文件存储。^^
